...
3. **Frontend**  
   ...
   - **NEXT_PUBLIC_API_URL (production):** Το frontend χρειάζεται το URL του deployed backend για login, chat, API. Αν δεν το ορίσεις, καλεί το placeholder `https://api.example.com` και παίρνεις *"Failed to fetch"*. **Τι να βάλεις:** κάνε deploy το backend (NestJS) somewhere (Railway, Render, Cloud Run, κ.λπ.)· το **public URL** που σου δίνει η υπηρεσία (π.χ. `https://your-backend.railway.app`) είναι το `NEXT_PUBLIC_API_URL`. Βάλ’ το στο **Vercel** → Project → **Settings** → **Environment Variables** (Production) και, προαιρετικά, στο **GitHub** → Repo → **Settings** → **Variables and secrets** → **Actions** → Variables, ώστε και το workflow να το περνάει στο build. Μετά κάνε **Redeploy** στο Vercel.
